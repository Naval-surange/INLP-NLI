...
class SNLIDataset(Dataset):
    ...
    def __init__(self, directory, prefix, bert_path, max_length: int = 512,random=False):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.random:
            np.random.shuffle(sentence_1_input_ids)
            np.random.shuffle(sentence_2_input_ids)
            sentence_1_input_ids = list(sentence_1_input_ids)
            sentence_2_input_ids = list(sentence_2_input_ids)
